 ...
 *  Open by default for students who haven't started; collapsed afterwards so it
 *  never blocks returning students. Server-only: the embed URL is signed here. */
export default function StartHere({ started }: { started: boolean }) {
  ...
  if (!src) return null
  return (
    <details className="panel" id="start-here" open={!started}>
      <summary style={{ cursor: 'pointer', listStyle: 'none' }}>
        <div className="eyebrow">Start here</div>
        <h2 style={{ margin: '6px 0 0' }}>▶ {started ? 'Rewatch: how this course works' : 'Watch this first: how to do this course'} <span className="muted">· 7 min</span></h2>
      </summary>

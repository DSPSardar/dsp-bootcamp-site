...
| Course schema | Existing `/mastery` graph updated in place, no second script: fuller Course `description`, curated nine-item `teaches`, `@id`s on the CourseInstance (`#instance`) and Offer (`#offer`), Offer price `"100.00"` and url → the landing page (was the noindex enrol page), DSP credential renamed "DSP Master Certificate" with a FAQ-mirrored description. Founder Person node gains two schema-only aliases via `founder.alsoKnownAs` | Kept, contrary to the requested block, per the Entity Lock (owner, 5 Sep): Person `@id` `/#sardar-ghaffar` (not `/about#…`), job title, one-sentence description (no "CEO"), `Organization` type (not `EducationalOrganization`), `logo.webp` as logo (og-card is the share image), full `sameAs` list. `test:schema` green |

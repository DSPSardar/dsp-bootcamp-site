...
| 4 links | Contextual links: `/` Proof sentence → `/mastery` ("self-paced AI agent course", `academy_cta_click {cta:'home_proof_inline'}`), Agentic Lab hero → `/mastery` ("the $100 self-paced version of this program"), `/mastery` instructor heading → `/about`, `/mastery` footer brand → `/` | Owner ruled out a "live bootcamp instead?" link on the buying page (sunset program). No nav change, no exact-match "AI agent course for beginners" anchor |
| 5 technical | robots already allows `/mastery`, disallows `/api/`, names the sitemap (no change). Sitemap: `/mastery` `lastModified` is now the last content commit on main (`PAGE_LAST_MODIFIED` in `mastery/seo.ts`, 2026-08-28) instead of `new Date()`; `/mastery/enrol` deliberately not listed (noindex). Certificate `<img>` → `next/image` (`fill` + `sizes`), instructor alt updated, no hero image so no `priority`. Bunny marketing embeds now sign with `preload=false` (lesson players unchanged); iframes already lazy, never autoplay, no poster exists. One `<h1>`, first heading on the page. All 7 `wa.me` CTAs on `/mastery` open in a new tab with `rel="noopener"` | Other pages' `wa.me` links and `new Date()` sitemap stamps left as they were (out of scope) |

## Perf pass — GA4 load gated on first interaction (2026-09-06)

| Unit | What | Notes |
|---|---|---|
| gtag.js | PageSpeed flagged ~980 KiB unused JS, 237 KiB unminified JS and 2.7 s JS execution on `/`. Measured live: our own Next chunks are ~554 KiB decoded and all minified; Google's `gtag.js` alone is 521 KiB decoded and is the source of nearly all of those flags. `strategy="lazyOnload"` did not help — the Lighthouse trace runs long enough to capture a post-`load` fetch. `src/components/site/GtagLoader.tsx` (client) now injects the `<script>` once, on the first `pointerdown` / `keydown` / `scroll` / `touchstart` (passive, once) or 8 s after `window.load`; under `navigator.connection.saveData` there is no timer, interaction only. The inline `ga4-init` script is unchanged (afterInteractive), so `window.gtag` + `dataLayer` exist from the start and every event queues until the library drains it | No dependency added. `src/lib/track.ts` untouched — it already queues into `dataLayer` when `gtag` is absent. Verified on `next start` with headless Chromium: zero requests to `googletagmanager.com` before a gesture, one after a scroll. Lab (no gesture) will now see no gtag at all; field visitors lose nothing beyond a few seconds of latency on the first hit |

## GEO pass — /llms.txt + /mastery Course schema (2026-09-06)

| Unit | What | Notes |
|---|---|---|
| llms.txt | The existing config-driven route (`src/app/llms.txt/route.ts`) gains "Programs & Services", "Core Course Specifications" (instructor, stack, core build, certifications, tuition, format) and "Links" (curriculum, FAQ, enrolment anchors). Tagline line added. Course copy comes from the new `COURSE_DESCRIPTION` in `mastery/seo.ts` | A `public/llms.txt` was requested; not added — a public file on the same path as a route is a Next build error, and the route keeps every fact on `site.ts`. Enrolment link points at `/mastery#pricing` (`#enrol` is not an anchor; `/mastery/enrol` is noindex) |
| Course schema | Existing `/mastery` graph updated in place, no second script: fuller Course `description`, curated nine-item `teaches`, `@id`s on the CourseInstance (`#instance`) and Offer (`#offer`), Offer price `"100.00"` and url → the landing page (was the noindex enrol page), DSP credential renamed "DSP Master Certificate" with a FAQ-mirrored description. Founder Person node gains two schema-only aliases via `founder.alsoKnownAs` | Owner follow-up ("fix them"): founder title → "Founder & Lead AI Instructor", description → "Founder and CEO … and its lead AI instructor" (both in `site.ts`, so bylines, /about, /sardar-ghaffar and every Person node agree); Organization node typed `['Organization', 'EducationalOrganization']` sitewide. Kept on purpose: Person `@id` `/#sardar-ghaffar` (one entity sitewide, not `/about#…`), `logo.webp` as logo (og-card is the 1200×630 share image, not a logo), full `sameAs` list, "Google-certified" (the credential is a certification). `test:schema` green |
